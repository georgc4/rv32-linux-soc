...
|---|---|---|---|
| M/S/U privilege and traps | `rv32_priv_unit`, core trap/return path | `test-priv` checks M-mode ECALL/CSR/MRET and timer IRQ; `test-supervisor` checks M-to-S transition and delegated S external IRQ | U-mode and broader delegation matrix; privileged ISA compliance |
