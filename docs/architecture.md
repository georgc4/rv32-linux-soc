...
Current implementation: `rtl/cpu/rv32i_core.v` has separate instruction and data request ports; `cpu_bus_adapter.v` joins them to the physical bus. The diagnostic core executes the main RV32I arithmetic, branch, jump, byte/halfword/word load and store encodings with aligned accesses. `FENCE` is serialized by the single-outstanding bus. `EBREAK` halts for the testbench. **This is not yet a compliant RV32I CPU:** architectural traps, `ECALL`, `FENCE.I`, CSRs, interrupts, M/A extensions, privilege and Sv32 are absent. Misalignment, illegal instructions and bus errors currently set a diagnostic fault output and stop. Full instruction and architectural tests remain ahead. Reuse is limited to test infrastructure and documented software tools unless separately proposed and accepted.
