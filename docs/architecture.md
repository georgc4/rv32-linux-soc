...
Current implementation: `rtl/soc/soc_top.v` connects a diagnostic CPU, interconnect, ROM, CLINT-like timer, UART, and serial memory bridge. `tt_um_rv32_linux_soc.v` maps these signals to the Tiny Tapeout logical interface. The CPU executes RV32I arithmetic, branches, jumps, aligned byte/halfword/word loads and stores, and RV32M arithmetic including specified divide edge cases. `FENCE` is serialized by the single-outstanding bus. `EBREAK` halts for the testbench. **This is not yet a Linux-capable CPU:** architectural traps, `ECALL`, `FENCE.I`, CSRs, interrupts, A extension, M/S/U privilege and Sv32 are absent. Misalignment, illegal instructions and bus errors currently set a diagnostic fault output and stop. The 32-bit multiply/divide operators currently infer large combinational logic and have not been area-optimized. Full instruction and architectural tests remain ahead.
