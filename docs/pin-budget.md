...
|---|---|---|
| Shared SI/SO for all five memories | `uio[0:1]` | High impedance while reset is held |
| PSRAM SIO2/SIO3 | `uio[2:3]` | External pull-downs required during power-up |
| NOR WP#/HOLD# | `uio[4:5]` | External pull-ups required during reset |
| Memory SCK | `uo[0]` | Low |
...
The split DQ2/DQ3 assignment resolves a startup conflict: PSRAM calls for these lines low during power-up, while NOR WP#/HOLD# must remain high for standard SPI. The bridge waits 150 µs at a 20 MHz clock, resets each PSRAM with `66h` then `99h`, and uses SCK at half the SoC clock. Its 64-bit read has a CS-low duration below the PSRAM 8 µs maximum at 20 MHz. The board must provide a stable 20 MHz clock, appropriate 3.3 V signaling and power, common ground, each device's decoupling, and the stated passive pulls. Pad drive/fanout and physical signal integrity remain unverified.
