...
|---|---|---|
| SKY26d schedule/area vs original Linux CPU | User has chosen SKY26d as the submission target. Logical RTL, directed tests, and an RV32 Linux image build exist, but there is no SBI firmware, Linux boot, mapped area result, or physical timing evidence. | Complete firmware/boot validation and mapped physical flow before submission. |
...
The integrated `synth-soc` run previously reported about **21,230 generic cells**. This cannot be converted to Sky130 tile count or timing. Map and route with the selected shuttle flow and leave routing margin.
